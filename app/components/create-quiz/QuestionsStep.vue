      ...
      <UCard v-if="showForm" class="space-y-4">
        ...
        <div>
          ...
          <URadioGroup
            ...
            :ui="{
              container: 'shrink-0',
              base: 'border border-gray-300 dark:border-gray-700 data-[state=checked]:border-primary-500 data-[state=checked]:bg-primary-500',
              indicator: 'bg-white',
              wrapper: 'flex-1',
              label: 'w-full'
            }"
          ...
          >
            <template #label="{ item }">
              <UInput
                :model-value="getOptionText(item.value)"
                placeholder="Answer text"
                size="xs"
                class="flex-1"
                @update:model-value="(value) => updateOptionText(item.value, value)"
              />
            </template>
              ...
              <div class="flex justify-end">
                <UTooltip text="Remove answer" :disabled="newQuestion.options.length <= 2">
                  <UButton
                    ...
                    @click.stop="removeOption(String(item.value))"
                  />
                </UTooltip>
              </div>
            </template>
...
const radioItems = computed(() =>
  newQuestion.options.map((option) => ({
    ...option,
    value: option.id,
    label: option.text || 'Answer',
    ui: {
      item: 'flex items-center gap-3 rounded-md border border-gray-200 px-3 py-2 dark:border-gray-800'
    }
  }))
);

const getOptionText = (optionId: string | number) => {
  const option = newQuestion.options.find((item) => item.id === optionId);
  return option?.text ?? '';
};

const updateOptionText = (optionId: string | number, text: string) => {
  const option = newQuestion.options.find((item) => item.id === optionId);
  if (option) {
    option.text = text;
  }
